...
| `npm run stage7:signoff:update -- --party <party> --reviewer <name> --date <YYYY-MM-DD> --decision <decision>` | Write a real signoff decision into `stage-7-signoff-record.md` | Use `||` in `--notes` to split multiple note lines; only write real reviewer / decision values |
