...
- `core-common` 已完成第一批共享基础抽离，`StateAdapter`、`StateHolderImpl`、`RefactorFeatureFlags / RefactorFeatureFlagDefaults` 接口层，以及 `DispatcherProvider / DefaultDispatcherProvider` 共享协程调度接口层已迁入共享层；当前只剩 `NovelUserDefaultsBackedRefactorFeatureFlags` 与 `OptimizedDispatcherProvider` 等宿主实现仍暂留 `app`。
...
- 当前未解决的 `core-common` 遗留：
  - `android/app/src/main/java/com/novel/core/config/RefactorFeatureFlags.kt`（当前仅剩 `NovelUserDefaultsBackedRefactorFeatureFlags` 实现）
