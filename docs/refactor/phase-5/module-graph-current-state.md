...
- `feature-reader` 已正式落地，并开始承载阅读器启动/恢复/动作 trace 协调器，以及 `ReaderReducer`、`ReaderStateAdapter`、`ReaderIntent / ReaderState / ReaderEffect`、`ReaderSettings`、`PageCountCacheData / ProgressiveCalculationState` 这类稳定阅读配置与分页状态契约。
