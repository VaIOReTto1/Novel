...
| Reader 翻页 | direct flip sample | 当前运行缺失 | 后续若要升级成硬门禁，需补 `FlipPageUseCase` 直接样本 | `已记录缺口` | `reader-performance-baseline-2026-03-21.md` |
| Reader 设置 | direct settings update sample | 当前运行缺失 | 后续若要升级成硬门禁，需补 `UpdateSettingsUseCase` 直接样本 | `已记录缺口` | `reader-performance-baseline-2026-03-21.md` |
...
| Baseline Profile | connected generation | 受设备 compile 阻塞 | 要么生成成功，要么保持“可复现阻塞项 + 补救路径” | `已接受阻塞项` | `baseline-profile-run-2026-03-21.md` |
| 设备 compile | `cmd package compile` on `com.novel` and `com.android.settings` | 两个包都失败 | 需在第二设备上复验，才能升级 compiled-mode 门禁 | `已接受阻塞项` | `device-compile-blocker-2026-03-21.md` |
