...
## 摘要
- 所属阶段：`Phase 6`
- 当前状态：`validated baseline package`
...
- 原因：
  ...
  - benchmark 外部直接执行 shell compile，也在：
    ...
    两个包上得到同样错误
- 结论：
  - 当前将其视为 `DN2101` 环境 blocker，而不是 `Phase 5` 或应用运行时回归

## Search / Reader / WebView / Host
- Search 已形成正式 log-sample baseline。
- Reader 已形成正式 init baseline，并显式写出 flip / settings 样本缺口。
...
- 同时也足以按既定规则关闭 `V6-02`：
  - 成功生成 profile，或
  - 形成可复现环境 blocker，并附带 remediation path
