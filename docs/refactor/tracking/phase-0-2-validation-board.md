...
| V1-06 | Phase 1 | 迁移演练矩阵完成至少一轮 | 升级风险可评估 | `docs/refactor/phase-1/migration-rehearsal-matrix.md` | 已补数据库、SharedPreferences、KeyChain 三类资产的迁移演练矩阵与执行模板，待补首轮实操证据 | in_progress | yellow | 待指定 | 待指定 | 2026-03-15 |
| V1-07 | Phase 1 | WebView 安全策略代码与文档一致 | 域名、SSL、外链、mixed content 规则明确 | `android/app/src/main/java/com/novel/page/welfare/component/WebViewComponent.kt`, `android/app/src/main/java/com/novel/page/welfare/utils/WelfareWebSecurityConfig.kt`, `android/app/src/main/java/com/novel/page/welfare/WelfarePage.kt`, `android/app/src/main/java/com/novel/page/welfare/viewmodel/WelfareMvi.kt`, `android/app/src/main/res/xml/network_security_config.xml`, `android/app/build.gradle`, `docs/refactor/evidence/welfare-bing-2026-03-15.png` | 已将福利页默认测试页切换为 `https://www.bing.com/`，补充 `bing.com/www.bing.com/cn.bing.com` 白名单，并完成真机验证：页面实际跳转到 `https://cn.bing.com/` 且 `OnPageFinished` 成功触发 | in_progress | green | 待指定 | 待指定 | 2026-03-15 |
| V1-08 | Phase 1 | signing、环境注入、依赖验证可追溯 | 发布与供应链基础完备 | `android/app/build.gradle`, `.gitignore`, `android/gradle/verification-metadata.xml` | 已启动 release signing 脚手架：支持从 Gradle 属性或环境变量读取签名参数，缺失时回退 debug signing 保持本地可构建；已生成 Gradle 依赖校验元数据并通过 `assembleRelease` 验证 | in_progress | yellow | 待指定 | 待指定 | 2026-03-15 |
