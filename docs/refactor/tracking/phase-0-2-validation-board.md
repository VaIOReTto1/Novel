...
| V2-06 | Phase 2 | CI 包含 assemble、detekt、lint、JVM tests、RN lint/test | 最小自动化门禁在线 | `jest.config.js`, `jest.setup.js`, `.eslintignore`, `__tests__/App.test.tsx`, `android/app/src/main/java/com/novel/core/adapter/StateAdapter.kt`, `android/app/src/main/java/com/novel/utils/network/NetworkMonitor.kt`, `android/detekt.yml` | `npm test -- --runInBand`、`./gradlew.bat app:testDebugUnitTest`、`./gradlew.bat app:lintDebug`、`./gradlew.bat :macrobenchmark:assemble` 已通过；`app:detekt` 已从配置阻塞恢复到真实扫描阶段，但当前仍有 `2260 weighted issues`，`npm run lint` 仍暴露大量 RN lint debt | `blocked` | `yellow` | 待指定 | 待指定 | 2026-03-15 |
