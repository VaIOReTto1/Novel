...
| V1-08 | Phase 1 | signing、环境注入、依赖验证可追溯 | 发布与供应链基础完备 | `android/app/build.gradle`, `.gitignore` | 已启动 release signing 脚手架：支持从 Gradle 属性或环境变量读取签名参数，缺失时回退 debug signing 保持本地可构建；当前 `assembleRelease` 验证通过 | in_progress | yellow | 待指定 | 待指定 | 2026-03-15 |
