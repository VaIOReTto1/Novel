...
| V17-03 | showcase infrastructure | RN Storybook 与 Android Showcase 可运行 | 已有 RN `Stage7Showcase.tsx` 与 web 显式入口开关，Android 已补 `Stage7ShowcaseScreen.kt` / `Stage7ShowcaseModel.kt` 骨架与 unit test，正式 Storybook 与 Android 导航接线仍待补齐 | `in_progress` | `yellow` |
