...
| V15-05 | governance drift report | catalog / smoke / registry / Figma frame 漂移可发现 | 已生成 `governance-drift-report.md` 与 `visual-planning-summary.md`，当前 registry drift 为 `none`、RN smoke tests 已扩到 `13` 条且 smoke catalog drift 为 `none`，Figma mapping 仍全部待补 | `in_progress` | `yellow` |
...
| V18-03 | visual regression and gates | 像素、无障碍、版权、性能门禁通过 | 已完成 `npm test -- --runInBand` 全量 Jest 回归，结果 `108` 个 suites / `260` 个 tests 全绿；已完成 Android 共享 gate `app:testDebugUnitTest app:lintDebug app:compileDebugAndroidTestKotlin :macrobenchmark:assemble`；视觉稿、无障碍、版权核验与性能收尾仍待继续补齐 | `in_progress` | `yellow` |
