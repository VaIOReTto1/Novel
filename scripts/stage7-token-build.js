...
const SOURCE_PATH = path.join('design-system', 'source', 'stage7.tokens.json');
...
const renderLess = (tokens) => {
  const lines = ['// Generated by scripts/stage7-token-build.js'];

  ['light', 'dark'].forEach((mode) => {
    flattenColorTokens(tokens, mode).forEach(({ key, value }) => {
      lines.push(`@stage7-${key.replace(/\./g, '-')}: ${value};`);
    });
  });

  Object.entries(tokens.space).forEach(([key, value]) => {
    lines.push(`@stage7-space-${key}: ${value}px;`);
  });
  Object.entries(tokens.radius).forEach(([key, value]) => {
    lines.push(`@stage7-radius-${key}: ${value}px;`);
  });
  Object.entries(tokens.motion.duration).forEach(([key, value]) => {
    lines.push(`@stage7-motion-duration-${key}: ${value}ms;`);
  });
...
const renderReactNative = (tokens) => {
  ...
  return `export const stage7LightTheme = ${toTypeScriptLiteral({
    color: lightColors,
    space: tokens.space,
    ...
    motion: tokens.motion,
    typography: tokens.typography,
  })} as const;\n\nexport const stage7DarkTheme = ${toTypeScriptLiteral({
    color: darkColors,
    space: tokens.space,
    radius: tokens.radius,
    elevation: tokens.elevation,
...
const renderAndroidXml = (tokens) => {
  ...
  ['light', 'dark'].forEach((mode) => {
    flattenColorTokens(tokens, mode).forEach(({ key, value }) => {
      lines.push(`  <color name="stage7_${xmlName(key)}">${value}</color>`);
    });
  });

  Object.entries(tokens.space).forEach(([key, value]) => {
    lines.push(`  <dimen name="stage7_space_${key}">${value}dp</dimen>`);
  });
  Object.entries(tokens.radius).forEach(([key, value]) => {
    lines.push(`  <dimen name="stage7_radius_${key}">${value}dp</dimen>`);
  });
